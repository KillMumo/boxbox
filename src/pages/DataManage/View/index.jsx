...
const View = (props) => {
  ...
  const [part3title, settitle] = useState('碳排放量(吨)');
  const [part3dataindex, setdataindex] = useState('total');
  const [menunum, setmenu] = useState(2);
  const [columnsNum, setcolumnNum] = useState(
    {
      title: '排名',
      width: 100,
      render: (t, r, index) => {
  ...
  const { searchBy, submit, exportExcel, excelLoading, tableProps } = useTable(
    getreportlistdata,
    {
      defaultFilters: {energyType: energyType2,industry:industry,year:year,scenario: 'carbon_rank_list'},
      form: props.form
    }
  )
  ...
  useEffect(()=>{
    ...
    if(role==='account_org'){
      searchBy({energyType: energyType2,year:year,industry:industry,companyName: companyName,scenario: 'carbon_rank_list'})
    }
    else searchBy({energyType: energyType2,year:year,industry:industry,scenario: 'company_summary'})
  },[energyType2])
    ...
    return(
      <div>
        ...
        <div className={styles.part2box}>
          ...
          <div className={styles.part2num}>{part1data.carbonOutCount}
          <img width="14" style={{marginLeft:'5px'}} src={iconMap[part1data.level]}/>
          </div>
        </div>
        <div className={styles.part2box}>
        ...
        <div className={styles.part2box}>
          ...
          <div className={styles.part2num}>{part1data.carbonGrade}
          <img width="14" style={{marginLeft:'5px'}} src={iconMap[part1data.nlevel]}/>
          </div>
        </div>
      </div>
        ...
        return (
          <div className={styles.part3box}>
            ...
            <div className={styles.part3num}>
              {el.num} <span className={styles.baifenbinew}>{el.num2}</span>
 ...
 return (
   <Card transparent >
     <Card style={{marginBottom:'16px'}} className={styles.companyname}> <div className={styles.pic4}></div>{companyName} | {year} | {industry}</Card>
     <Card title="账户概览" style={{marginBottom:'16px'}}>{part1()}</Card>
     <Card title="碳诊断报告" >{part2()}</Card>
     {(industry!=='-')&&(<Card title="本行业碳排分析">{part3()}</Card>)}
     <Row>
       <Col span={12}><Card title="碳排来源分布" style={{marginBottom:'16px',marginRight:0}}>{part4()}</Card></Col>
       {(industry!=='-')&&(<Col span={12}><Card title="本行业碳排来源分布" style={{marginBottom:'16px',marginLeft:0}}>{part5()}</Card></Col>)}
     </Row>
   </Card>
 )
}
