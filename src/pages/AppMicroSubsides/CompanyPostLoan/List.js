...
import { fetchCompanyPostLoanList, fetchCompanyExcel } from '@/services/microSubsidy/postLoan'
...
const List = (props) => {
  ...
  const columns = useMemo(() => {
    const getActionMap = (r) => ({
      待还款: {
        name: '查看',
        props: { to: `/msCompanyPostLoan/management/view/${r.bizNo}` }
      },
      逾期未还清: { name: '查看', props: { to: `/msCompanyPostLoan/management/view/${r.bizNo}` } },
      逾期已还清: {
        name: '查看',
        props: { to: `/msCompanyPostLoan/management/view/${r.bizNo}` }
      },
      已还清: { name: '查看', props: { to: `/msCompanyPostLoan/management/view/${r.bizNo}` } }
    })
  ...
  const renderSearchForm = () => {
    const handleSelect = (v) => {
      router.push(`/msCompanyPostLoan/management/${v}`)
    ...
    const handleReset = () => {
      router.push(`/msCompanyPostLoan/management/全部`)
      resetFields()
  ...
  return (
    <Card transparent>
      ...
      <Card style={{ marginTop: -10 }}>
        <PagingTable rowKey={(i) => i.id} scroll={{ x: 1230 }} columns={columns} {...tableProps} />
      </Card>
    </Card>
  )
}
