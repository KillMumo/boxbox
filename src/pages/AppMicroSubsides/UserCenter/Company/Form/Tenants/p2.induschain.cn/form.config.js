...
import { IDCardReg, PhoneReg, BankCodeReg } from '@/common/reg'
import { InputMoney, InputPrice } from '@/components/FormItems'
import { validateFiles, validateMonth } from '@/utils/fieldValidator'
import UploadButton from '@/pages/AppMicroSubsides/components/UploadButton'
...
export const useBasicConfig = (form, initialValue = {}) => {
  ...
  return [
    ...
    },
    {
      ...
      children: (
        <Upload
          showOnly={!!initialValue[renderCompanyName('businessLicenseUrl')]}
          {...getUploadProps('企业营业执照正本、副本或复印件', true)}
    ...
    },
    {
      ...
      children: (
        <Upload
          showOnly={!!initialValue[renderCompanyName('organizationRuleUrl')]}
          {...getUploadProps('企业章程', true)}
        />
      )
    },
    {
      itemProps: { label: '测试新增字段', wrapperCol: { span: 14 } },
      rules: [{ required: true, message: '请上传测试新增字段' }, { validator: validateFiles }],
      name: renderCompanyName('testFile'),
      validateTrigger: 'onChange',
      children: (
        <Upload
          showOnly={!!initialValue[renderCompanyName('testFile')]}
          {...getUploadProps('加盖公章的测试新增字段', true)}
        />
      )
    },
    {
    ...
    },
    {
      ...
      children: (
        <Radio.Group disabled onChange={(e) => toggleIsRealControl(!!e.target.value)}>
...
export const useBankConfig = (form) => {
  ...
  const {
    ...
    provinceLabel: renderConfigName('openProvince')
  })
  ...
  return [
    {
      ...
      children: (
        <Select disabled placeholder="请选择" {...bankProps}>
          {bankList.map((i) => (
      ...
      children: [
        {
          ...
          children: (
            <Select disabled {...provinceProps} placeholder="请选择">
              {provinceList.map((p) => (
        ...
        },
        {
          ...
          layout: { container: ColLayout },
          children: (
            <Select disabled {...cityProps} placeholder="请选择">
              {cityList.map((c) => (
    ...
    },
    {
      ...
      children: (
        <Select disabled {...branchProps} placeholder="请选择">
          {branchList.map((i) => (
...
export const useBankNormalConfig = (form, id) => {
  ...
  // const renderConfigName = (name) => renderName('normalList', name)
...
export const useFamilyConfig = (form, relation) => {
  ...
  return [
    ...
    },
    {
      ...
      condition: { name: '身份证号', regex: { regex: IDCardReg } },
      name: renderConfigName('idCard'),
      children: <Input placeholder="请输入" />
    },
    {
      itemProps: { label: '手机号' },
      condition: { name: '手机号', regex: { regex: PhoneReg } },
