...
import {
  addMethods,editMethods
} from '@/services/carbonAccount/device'
import {getmethoddata} from '@/services/carbon/dataManage'
...
const Hist = (props) => {
  ...
  // 申请添加
  const { loading: submitLoading, start: submitReq } = useRequest(editMethods, {
    manual: true,
    onSuccess: () => {
      ...
      <Form {...formLayout} >
        <Form.Item label="方法学名称" >
          {getFieldDecorator('methodName',
           {
            initialValue: getmethoddata().extra?.methodName,
            rules: [
              {
                required: true,
                message: '请输入方法学名称！'
        ...
        <Form.Item label="适用场景">
          {getFieldDecorator('methodScene', {
            initialValue: getmethoddata().extra?.methodScene,
            rules: [
              {
                required: true,
                message: '请输入适用场景！'
        ...
        <Form.Item label="描述">
          {getFieldDecorator('methodDesc', {
            initialValue: getmethoddata().extra?.methodDesc,
            rules: [
              {
                required: true,
                message: '请输入方法学描述！'
        ...
        <Form.Item label="方法学版本">
          {getFieldDecorator('methodFile',{
            initialValue: getmethoddata().extra?.methodFile,
            rules: [
              {
                required: true,
                message: '碳排计算模型！'
